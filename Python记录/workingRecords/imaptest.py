    ...
    for item in msg.walk():
        if item.get_filename():
            ...
            if dh[0][1]:  # 如果包含编码的格式，则按照该格式解码
                print(dh[0][1])
                filename = filename.decode(dh[0][1]).encode('utf8')
                if 'big5' in filename.decode('utf8'):
                    file_name = email.header.decode_header(filename.decode('utf8'))[0][0].decode('big5')
                    print(file_name)
                    file_name = file_name.encode('utf8')
                # filename = file_name.decode(dh[0][1])
